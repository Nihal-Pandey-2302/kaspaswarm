...
**[▶ Demo video](https://youtu.be/UKgoAD2cslM) · [🌐 Live demo](https://kaspaswarm.vercel.app/) · Run locally in one command: `docker compose up`**
...
broker** that decides who works and who gets paid — a single point of trust and
failure. And once agents hold funds, nothing stops a buggy or compromised agent from
draining its wallet.
...
and settle — the blockchain is simultaneously the **message bus** (each message rides
in the transaction `payload`) and the **payment rail**. Agent treasuries are then put
under **on-chain covenants** so spending policy is enforced by consensus, not by trust.

This only works because Kaspa's blockDAG confirms **~10 blocks per second** — fast
enough to bid → assign → verify → settle in real time, which Bitcoin (10 min) or
Ethereum (12 s) cannot. Combined with Kaspa's **KIP-10 introspection opcodes**, it lets
us turn *"trust the agent"* into *"the chain won't let it."*

## Highlights

- **Real on-chain coordination** on testnet-10 via the official Kaspa SDK + community-node Resolver — delivery is gated on real block inclusion, so turning the chain off stops the swarm.
...
- **MCP-hireable** — any external agent (Claude Desktop, Cursor) can hire the swarm as a tool: post a task, the swarm solves + settles on Kaspa, and returns the answer.
- **On-chain covenant governance (KIP-10)** — an Agent Treasury Vault whose spend policy Kaspa consensus enforces; an over-cap or off-policy spend is *rejected by the network*. [Details ↓](#on-chain-covenant-governance)
- **One-command run & deploy** — Docker Compose, a Render blueprint, tests + CI, and a fund-safe idle guard for live deployments.
...
*Swarm performance — tasks created over time and task-outcome breakdown.*
...
- 🌐 **Resolver** — connect to testnet-10 through community nodes; no node to run.
...
> **Honest scope.** These are **testnet-10 proofs of concept** — Toccata / SilverScript
...
The fastest path — no Python/Node/wallet needed:
...
- **Transport** (`backend/kcore/sdk_transport.py`) — Kaspa SDK via the Resolver:
  streams `BlockAdded`, decodes `KSWARM1:` payloads, builds change-aware transactions
  with adaptive mass-based fees. A hand-rolled wRPC `ChainWatcher` is the fallback.
...
- **Frontend** — React + raw Three.js dashboard streaming live state over WebSocket.
...
Hackathon EP5**, with substantial new work: the SDK+Resolver transport, real LLM agents +
verifier, the reputation-auction + escrow economy, the MCP server, **three on-chain
covenants**, and one-command Docker/Render deploy with tests and CI. Reuse with
significant new work was confirmed by Kaspa core (IzioDev).
